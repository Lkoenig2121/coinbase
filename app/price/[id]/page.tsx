...
export default function CryptoDetailPage() {
  ...
  const [activeTab, setActiveTab] = useState<'balance' | 'insights'>('insights')
  ...
  const fetchPriceHistory = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:3001/api/crypto/${id}/history?days=${timeRange}`)
      
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}))
        console.error('Failed to fetch price history:', response.status, errorData)
  ...
  const renderChart = () => {
        ...
        <div className="h-64 bg-gray-50 rounded-lg flex items-center justify-center">
          <div className="text-center text-gray-500">
            ...
            <p className="text-xs mt-2 text-gray-400">
              {chartLoading ? 'Loading...' : 'Make sure the Express server is running on port 3001'}
            </p>
          </div>
        </div>
      )
    ...
    const areaPath = `${pathData} L ${lastPoint.x},${height - padding} L ${padding},${height - padding} Z`
          ...
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center space-x-4">
              ...
              <nav className="hidden md:flex space-x-6">
                <Link href="/home" className="text-gray-700 hover:text-coinbase-blue font-medium">Home</Link>
                <Link href="/trade" className="text-gray-700 hover:text-coinbase-blue font-medium">Trade</Link>
              </nav>
            </div>
            <div className="flex items-center space-x-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <main className="lg:col-span-9">
            ...
            <div className="mb-6">
              <div className="flex items-center space-x-3 mb-4">
                ...
                <h1 className="text-3xl font-bold text-gray-900">{crypto?.name || 'Loading...'}</h1>
              </div>
            </div>
            ...
            <div className="bg-white border border-gray-200 rounded-lg p-6 mb-6">
              <div className="flex items-baseline space-x-4 mb-4">
                <div className="text-4xl font-bold text-gray-900">{formatPrice(crypto?.current_price)}</div>
                <div className={`flex items-center space-x-2 text-lg font-semibold ${
                  ...
                  <span>{formatChange(priceChange)} ({formatChange(priceChangePercent)}%)</span>
                </div>
              </div>
              ...
              <div className="flex space-x-2">
                {[
                  ...
                  const isMax = index === 5
                  const isSelected = isMax ? timeRange === '365' : timeRange === range.value
                  return (
                    <button
                      key={index}
            ...
            {activeTab === 'balance' && (
              <div className="bg-white border border-gray-200 rounded-lg p-6">
                ...
                <div className="text-center py-8">
                  ...
                  <div className="text-gray-600 mb-4">0 {crypto?.symbol?.toUpperCase() || ''}</div>
              ...
              <div className="space-y-6">
                  ...
                  <div className="flex items-start space-x-3 mb-4">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center space-x-2 mb-2">
                        <span className="text-xs font-medium text-gray-500">Happening now</span>
                        <span className="text-xs text-gray-400">•</span>
                        <span className="text-xs text-gray-400">AI generated 1h ago</span>
                      </div>
                      <p className="text-gray-900">
                        After declining roughly {Math.abs(priceChangePercent).toFixed(0)}% over the past week, {crypto?.symbol?.toUpperCase() || 'crypto'} traders are watching whether a significant surge in trading activity can help stabilize the {isPositive ? 'rising' : 'falling'} price.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white border border-gray-200 rounded-lg p-6">
                  <h4 className="font-semibold text-gray-900 mb-3">
                    {isPositive ? 'Heavy buying' : 'Heavy selling'} meets increased activity
                  </h4>
                  <p className="text-gray-700 leading-relaxed">
                    {crypto?.name || 'This cryptocurrency'} has {isPositive ? 'risen' : 'dropped'} roughly {Math.abs(priceChangePercent).toFixed(0)}% over the past week even as trading volume {isPositive ? 'increased' : 'jumped'} approximately {Math.floor(Math.random() * 50 + 50)}% in the past 24 hours and unique traders increased by around {Math.floor(Math.random() * 50 + 50)}%, showing more people are getting involved {isPositive ? 'with' : 'despite'} the price.
                  </p>
                </div>
              </div>
            )}
          ...
          <aside className="lg:col-span-3">
            <div className="bg-white border border-gray-200 rounded-lg p-6 sticky top-20">
              ...
              <div className="mb-6">
                <div className="text-3xl font-bold text-gray-400 mb-2">0 USD</div>
                <div className="flex items-center justify-end mb-2">
                  <button className="text-xs text-coinbase-blue hover:underline">Max</button>
                </div>
                <div className="text-sm text-coinbase-blue">0 {crypto?.symbol?.toUpperCase() || ''}</div>
              </div>
                ...
                <div className="p-4 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-sm font-medium">Select a payment method</span>
                    </div>
